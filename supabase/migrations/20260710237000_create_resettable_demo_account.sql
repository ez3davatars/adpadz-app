-- Owner-operated, resettable showcase account.
--
-- This does not create an Auth user and does not expose shared credentials.
-- A service-role provisioning script creates or updates the dedicated Auth
-- user, marks its app_metadata as a demo account, and registers its UUID in
-- this private, service-role-only registry. The registered user can then reset
-- only its own tenant to the fixed River City fixture.

CREATE TABLE IF NOT EXISTS public.demo_accounts (
  user_id uuid PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  email text,
  fixture_key text NOT NULL DEFAULT 'river-city'
    CHECK (fixture_key = 'river-city'),
...
COMMENT ON TABLE public.demo_accounts IS
  'Private service-role registry for owner-operated demo accounts. Contains no passwords or access tokens.';

ALTER TABLE public.demo_accounts ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON TABLE public.demo_accounts FROM PUBLIC;
REVOKE ALL ON TABLE public.demo_accounts FROM anon;
REVOKE ALL ON TABLE public.demo_accounts FROM authenticated;
...
AS $$
  SELECT
    ...
    AND COALESCE(auth.jwt() -> 'app_metadata' ->> 'is_demo', 'false') = 'true'
    AND EXISTS (
      SELECT 1
      FROM public.demo_accounts AS demo
      WHERE demo.user_id = auth.uid()
...
REVOKE ALL ON FUNCTION public.is_demo_account() FROM anon;
GRANT EXECUTE ON FUNCTION public.is_demo_account() TO authenticated;

COMMENT ON FUNCTION public.is_demo_account() IS
  'True only for the current authenticated user when both protected app_metadata and the private registry mark it as an active demo account.';
...
REVOKE ALL ON FUNCTION public.reset_demo_workspace() FROM anon;
GRANT EXECUTE ON FUNCTION public.reset_demo_workspace() TO authenticated;

COMMENT ON FUNCTION public.reset_demo_workspace() IS
  'Atomically replaces only the current registered demo owner tenant with the fictional River City showcase and returns its current public paths.';

-- Intentionally not seeded: Cloudflare/provider image IDs, uploaded files,
-- hosted video, real booking integrations, outbound email/social delivery,
-- documents, and real customer PII. Those require external credentials or
-- durable third-party assets and should never be fabricated by a demo reset.
