...
type Props = {
  ...
  /**
   * The campaign's saved Mailer creative settings, resolved by the surface
   * that can read them (Mission Control). When absent, the presentation
   * falls back to synthesized defaults (public and business surfaces).
   */
  creativeSettings?: CampaignTemplateSettings | null;
};
export default function CommunityMailerPlacement(
  ...
  return (
    <button
      type="button"
      tabIndex={mode === "print-preview" || !selectable ? -1 : 0}
      ...
      {creative
        ? (
          placement.campaign_id ? <CampaignTemplateRenderer
            ...
            settings={creativeSettings ?? normalizeTemplateSettings({ template: placement.is_featured ? 'featured-sponsor' : 'hero-visual', showQr: Boolean(placement.qr_destination_url), showExpiration: false })}
